...
export default function OrderModal({ isOpen, onClose, product }: OrderModalProps) {
  const [formData, setFormData] = useState({ date: '', recipient: '', address: '', message: '' });
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
        ...
        const { data: products } = await supabase.from('products').select('*').neq('id', product.id).order('created_at', { ascending: false }).limit(3); 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Format Tanggal (Dari YYYY-MM-DD menjadi 15 Januari 2026)
    ...
    // Format Pesan WhatsApp Polos (Tanpa Emoji)
    const messageLines = [
      ...
      `"${formData.message || '-'}"`,
      "----------------------------------------",
      "Mohon konfirmasi pesanan & total ongkirnya ya kak. Terima kasih!"
    ];
          ...
          >
            ...
            <div className="overflow-y-auto p-5 space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      required 
                      min={minDate} 
                      className="w-full pl-10 p-2.5 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 focus:ring-2 focus:ring-rose-500 outline-none" 
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Nama Penerima</label>
                  <div className="relative">
                    <User className="absolute top-2.5 left-3 text-gray-400" size={16} />
                    <input type="text" required placeholder="Contoh: Dinda Permatasari" className="w-full pl-10 p-2.5 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 focus:ring-2 focus:ring-rose-500 outline-none" onChange={e => setFormData({...formData, recipient: e.target.value})} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative mb-4">
                    ...
                    <textarea 
                      required
                      rows={2}
                      placeholder="Sebutkan Alamat lengkap, Jalan, Desa/Kelurahan, Kab/Kota, Provinsi..." 
                      className="w-full pl-10 p-2.5 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 focus:ring-2 focus:ring-rose-500 outline-none" 
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <textarea maxLength={150} rows={2} placeholder="Ucapan singkat..." className="w-full pl-10 p-2.5 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 focus:ring-2 focus:ring-rose-500 outline-none" onChange={e => setFormData({...formData, message: e.target.value})} />
                  </div>
                  
                  {/* LIVE PREVIEW KARTU UCAPAN */}
                  <div className="mt-3 p-4 bg-[#f8f5f0] border border-[#e8dfce] rounded-lg shadow-inner relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-2 bg-[url('https://www.transparenttextures.com/patterns/cream-paper.png')] opacity-30"></div>
                    <p className={`text-center text-gray-800 leading-relaxed text-lg min-h-[4rem] flex items-center justify-center italic ${dancingScript.className}`}>
                      {formData.message ? `"${formData.message}"` : '"Pratinjau kartu ucapan akan muncul di sini..."'}
                    </p>
                  </div>
                </div>
